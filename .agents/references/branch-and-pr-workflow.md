...
Create the feature branch immediately after the pre-execution checkpoint is accepted, before any
execution agent is spawned:

- default: a single branch `work/<feature-slug>` off the current branch
- record it in `docs/<feature>/plan.yaml` under `branching.branch_name`
